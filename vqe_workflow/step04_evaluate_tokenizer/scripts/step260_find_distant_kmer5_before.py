...
def find_most_distant_subset(args):
    ...
    # 2. 提取维度列
    dim_cols = ['dim0', 'dim1', 'dim2', 'dim3']
    if not all(col in df.columns for col in dim_cols):
        print(f"❌ 错误: CSV 中缺少必要的维度列 {dim_cols}")
        return

    # 3. 按 category 聚合，计算质心
    ...
    for _ in range(args.top_n - 2):
        ...
        # 计算剩余点到已选集合的“代表性距离”
        # 这里使用：到已选集合中所有点的最小距离的最大化 (Max-Min Strategy)
        # 这种策略能保证选出的点分布最散
    ...
    # 打印列表，方便直接复制到 required_base_patterns
    print(f"\n字符串格式 (可以直接复制):")
    ...
    for i, idx in enumerate(selected_indices, 1):
        ...
        if other_indices:
            ...
            print(f"   {i}. {cat_name:10} | 坐标: [{vec[0]:.3f}, {vec[1]:.3f}, {vec[2]:.3f}, {vec[3]:.3f}] | 平均间隔距离: {avg_dist:.6f}")
        else:
            print(f"   {i}. {cat_name:10} | 坐标: [{vec[0]:.3f}, {vec[1]:.3f}, {vec[2]:.3f}, {vec[3]:.3f}]")
