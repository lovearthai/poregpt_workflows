...
def process_token_base_counts(input_path, output_path):
        ...
        with gzip.open(input_path, 'rt', encoding='utf-8') as f:
            for line in tqdm(f, desc="处理进度"):
                try:
                    ...
                    high_tokens = [pair[0] for pair in layered_data]

                    # 提取对应的碱基
                    bases = data.get('tokens_based', [])

                    # 使用 zip 将两者配对并更新计数器
                    for t_id, base in zip(high_tokens, bases):
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for idx, ((t_id, base), count) in enumerate(sorted_items, 1):
            # 计算占比并保留 9 位小数
            ratio = round(count / total_count, 9)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='统计 Token 高位值与碱基的组合频次并计算占比')
