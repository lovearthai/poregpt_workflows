...
def plot_dimensions_hist(args):
    ...
    # --- 修改部分：解析 feature 字段 ---
    print("正在从 feature 字段提取维度数据...")
    # 将 "val1_val2_val3_val4" 拆分为 4 列，并转换为 float
    features_split = df['feature'].str.split('_', expand=True).astype(float)
    
    # 定义维度列名
    feature_cols = [f'dim{i}' for i in range(features_split.shape[1])]
    features_split.columns = feature_cols
    
    # 将拆分后的维度合并回主 DataFrame
    ...
    # 3. 绘图展示 (2行2列布局)
    # 注意：这里 feature_cols 已经在上方动态生成了，通常是 ['dim0', 'dim1', 'dim2', 'dim3']
    ...
    for i, col_name in enumerate(feature_cols[:4]): # 仅绘制前4个维度以适配 2x2 布局
        row, col = i // 2, i % 2
        ax = axes[row, col]
