...
# 【新增】定义输出目录
OUT_DIR="step0x3_fsq_base_distribution"
...
if [ -n "$2" ]; then
    ...
    OUTPUT_FILE="${OUT_DIR}/step0x3_fsq_base_distribution.png"
fi
...
# Python 脚本名称
PYTHON_SCRIPT="scripts/step0x3_fsq_base_distribution_scatter_plot_movetable_jsonlgz.py"
...
if [ $? -eq 0 ]; then
    echo -e "\n✅ 可视化任务成功结束！"
    echo "📊 高层/低层坐标的碱基分布已保存至: $OUTPUT_FILE"
else
    echo -e "\n❌ 执行失败，请检查报错日志。"
    exit 1
fi